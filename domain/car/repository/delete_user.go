...
func (lr carRepository) DeleteCarRepository(ctx context.Context, id int) (err error) {

	tx := lr.Database.DB.MustBegin()
	stmt, err := tx.PrepareContext(ctx, "UPDATE car SET deleted_at = now(), updated_at = now() WHERE id = ?")
	if err != nil {
