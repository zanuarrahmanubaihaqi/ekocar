...
func (lr userRepository) GetUserListsRepository(ctx context.Context, limit, offset int, sortby, search string) (products []model.User, err error) {
	...
	query := fmt.Sprintf("SELECT * FROM User WHERE deleted_at IS NULL %s  ORDER BY %s LIMIT $1 OFFSET $2", search, sortby)
...
func (lr userRepository) GetUserListsWithFiltersRepository(ctx context.Context, filter *shared_model.Filter, offset int) (products []model.User, err error) {
	...
	query, err := query.SelectStatementBuilder(model.User{}, "product", filter)
	if err != nil {
